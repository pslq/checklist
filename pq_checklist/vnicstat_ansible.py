...
DOCUMENTATION = r'''
  Simple module to get vnicstat for all interfaces on the server
'''
  ...
  if 'aix' in os.sys.platform :
    ...
    if rc == 0 :
      for l in stdout.split('\n') :
        lp = l.strip().split(':')
        if len(lp) == 4 :
          if any([ True for st in ( 'vnicserver' ) if st in lp[0]]) and lp[1] in ( "if", "adapter" ) :
            result['adapters'].append(lp[0])

    # Get Stats for these adapters
    for adpt in result['adapters'] :
      rc, stdout, stderr = module.run_command("/usr/sbin/vnicstat -d %s -n"%adpt)
      if rc == 0 :
